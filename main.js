...
async function renderRecipient(token) {
  const { data, error } = await supabase.rpc("get_public_coupon", { p_token: token });
  if (error || !data?.length) {
    setApp(`<section class="card narrow"><h1>❌ Kupon tidak ditemukan</h1><p>Link kupon tidak valid.</p><a class="button primary" href="/">Kembali</a></section>`);
    return;
  }
  const c = data[0];
  setApp(`
  ...
  `);
  await QRCode.toCanvas(document.querySelector("#qr"), c.token, { width:260, margin:2 });
}
